...
Activate `md-log` pointing to the session log file. It auto-logs every assistant message.
...
If `--model` was provided, switch to that model. If the model is not available, warn and
continue with current.

### Step 0.4 — Set up the session log

1. Create the log file: `<workspace>/<domain>/logs/YYYY-MM-DD-<topic-slug>.md`
2. Activate md-log: `/md-log <log-path>`
...
4. **Log** — md-log handles this automatically.
