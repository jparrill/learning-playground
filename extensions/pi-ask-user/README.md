...
A [pi coding agent](https://github.com/badlogic/pi-mono) extension that gives the LLM a structured, interactive way to ask questions — for clarification, preferences, diagnosis, learning checks, and surveys.

The tool pauses, presents choices, and waits for answers. It only collects responses; it does not score or correct them.
...
The LLM will call `ask_user` with structured questions. You answer in the TUI. The answers are returned to the LLM and it continues.
...
```typescript
{
  title?: string;            // Optional name for persistent logs
  questions: Array<{
...
| `Tab`            | Regular answer option         | Open detail editor         |
| `Space` or `Tab` | "Type something..." row       | Open free-text editor      |
...
- **Option details** — press `Tab` on any regular answer option to add detail. Result uses `Option (detail)`; in multi-select, each detail stays beside its option. `Type something...` keeps its free-text presentation.
...
tests/
  component.test.ts — 112 unit tests (vitest)
```
